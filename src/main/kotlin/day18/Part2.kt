...
object Part2 {
    ...
    fun load(input: List<String>): List<Instruction> {
        return input.map {
            ...
            val direction = hex.get(7)
            Instruction(Direction.from(direction), distance, s[2])
        }
    }

    data class Instruction(val direction: Direction, val length: Long, val colour: String)
    ...
    enum class Direction(val x: Int, val y:Int) {
        ...
        companion object {
            fun from(character: Char): Direction {
